...
class ExtendedClient extends Client {
    ...
    public async init(): Promise<void> {
        ...
        fs.readdirSync(buttonsPath).forEach((dir) => {
            const buttonFiles = readdirSync(`${buttonsPath}/${dir}`).filter((file) => file.endsWith(".js") || file.endsWith(".ts"));
        ...
        fs.readdirSync(slashPath).forEach(async (dir) => {
            const slashCommmands = readdirSync(`${slashPath}/${dir}`).filter((file) => file.endsWith(".js") || file.endsWith(".ts"));
