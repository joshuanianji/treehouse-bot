...
use songbird::{
    ...
    Bitrate, Call, Event, EventContext, EventHandler as VoiceEventHandler, SerenityInit,
    TrackEvent,
};
...
#[group]
#[commands(deafen, join, leave, mute, ting, undeafen, unmute, sheesh)]
struct General;
...
#[command]
#[only_in(guilds)]
async fn sheesh(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    let guild = msg.guild(&ctx.cache).await.unwrap();
    let guild_id = guild.id;
    ...
    let connect_to = match channel_id {
        ...
        None => {
            // user is not in a vc
            check_msg(msg.channel_id.say(&ctx.http, "SHEEEEEESH").await);
            return Ok(());
        }
    };
    ...
    if let Ok(_reader) = success_reader {
        ...
        check_msg(msg.channel_id.say(&ctx.http, "Sheesh!").await);
